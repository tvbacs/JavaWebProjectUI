...
function OrderHistory() {
  ...
  // Block admin from accessing order history
  if (user?.type === 'admin') {
    return (
      <div className={cx("wrapper")}>
        <div className={cx("content")} style={{ textAlign: 'center', padding: '50px' }}>
          ...
          <p style={{ color: '#6c757d', marginBottom: '30px' }}>
            Tài khoản Admin không có lịch sử đơn hàng cá nhân. <br/>
            Vui lòng sử dụng trang quản lý đơn hàng trong Admin Panel.
          </p>
          <Link to="/admin/orders" className={cx("btn-shop-now")} style={{ display: 'inline-block', textDecoration: 'none', marginRight: '10px' }}>
            Quản lý đơn hàng
          </Link>
          <Link to="/" className={cx("btn-shop-now")} style={{ display: 'inline-block', textDecoration: 'none' }}>
            Về trang chủ
          </Link>
        </div>
      </div>
