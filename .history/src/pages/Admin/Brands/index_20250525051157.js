...
function AdminBrands() {
  ...
  const fetchBrands = async () => {
    try {
      setLoading(true);
      // Try to get brands from real API first
      const brandResult = await electronicService.getAllBrands();
      if (brandResult.success && brandResult.data.length > 0) {
        setBrands(brandResult.data);
        console.log('🏷️ Real brands loaded:', brandResult.data.length);
      } else {
        // Fallback: Get brands from electronics data
        ...
        if (electronicsResult.success) {
          ...
          console.log('🏷️ Brands extracted from electronics:', uniqueBrands.length);
        }
      }
    } catch (error) {
    ...
    <div className={cx("wrapper")}>
      ...
      <div className={cx("search-section")}>
        <div className={cx("search-input-wrapper")}>
          ...
          <input
            type="text"
            placeholder="Tìm kiếm thương hiệu... (tự động tìm kiếm)"
            value={searchKeyword}
