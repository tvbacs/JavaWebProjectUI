...
const adminService = {
  ...
  createUser: async (userData) => {
    try {
      ...
      // TODO: Remove mock when backend implements POST /admin/users
      ...
      if (USE_MOCK) {
        ...
        return {
          ...
          data: {
            ...
            message: `User ${userData.fullname} đã được tạo thành công`
          },
        };
      }
  ...
  updateUser: async (userId, userData) => {
    try {
      ...
      console.log('🔄 Updating user via API:', userId, userData);

      // TODO: Remove mock when backend implements PUT /admin/users/{id}
      const USE_MOCK = true; // Set to false when backend is ready
      ...
      if (USE_MOCK) {
        ...
        return {
          ...
          data: {
            userId: userId,
            ...userData,
            message: `User ${userId} đã được cập nhật thành công`
          },
        };
      }
  ...
  deleteUser: async (userId) => {
    try {
      ...
      // Get user data to check if it's an admin
      const usersResult = await adminService.getAllUsers();
      if (!usersResult.success) {
        return usersResult;
      }
      ...
      // Mock successful deletion since backend doesn't have user management endpoints yet
