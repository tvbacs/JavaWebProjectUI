...
const adminService = {
  ...
  getUserStatistics: async () => {
    try {
      ...
      // Get users from getAllUsers method (which uses mock data)
      ...
      const userStats = {
        ...
        newUsersThisMonth: Math.min(2, users.length), // Mock data
  ...
  getAllUsers: async () => {
    try {
      ...
      // Mock user data since /auth/all-users endpoint doesn't exist
      const mockUsers = [
        {
          userId: 'admin001',
      ...
      const message = error.response?.data?.error || error.message || 'Không thể lấy danh sách người dùng';
      return { success: false, message };
    }
  },
