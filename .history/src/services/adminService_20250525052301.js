...
const adminService = {
  ...
  updateProductStock: async (productId, quantity) => {
    try {
      // Use the new stable endpoint for updating product stock
      ...
      console.error('❌ Error updating product stock with new endpoint:', error);

      const message = error.response?.data?.error || error.response?.data?.message || 'Không thể cập nhật số lượng sản phẩm';
      return { success: false, message };
    }
  },
