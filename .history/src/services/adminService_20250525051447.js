...
const adminService = {
  ...
  updateProductStock: async (productId, quantity) => {
    try {
      ...
      // Use the real electronics update API
      const response = await request.put(`/electronics/${productId}`, updateData);
      ...
      return {
        success: true,
        data: response.data,
        message: `Đã cập nhật số lượng từ ${currentProduct.quantity} thành ${quantity}`
      };
    } catch (error) {
      console.error('❌ Error updating product stock:', error);

      // If real API fails, use mock update for demo
      if (error.response?.status === 404 || error.response?.status === 500) {
        ...
        return {
          ...
          message: `Mock: Đã cập nhật số lượng thành ${quantity}`,
          isMock: true
        };
      }
